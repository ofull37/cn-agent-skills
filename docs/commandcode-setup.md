...
```bash
cmd skills add ofull37/cn-agent-skills
```
...
```bash
cmd skills add ofull37/cn-agent-skills -s spec-driven-development
```

**User scope** (installs into `~/.commandcode/skills/`, available in every project):

```bash
cmd skills add ofull37/cn-agent-skills --global
```

**Other supported forms:**

```bash
cmd skills add ofull37/cn-agent-skills@main            # a specific branch
cmd skills add ofull37/cn-agent-skills/skills/interview-me   # a specific path in the repo
cmd skills add ofull37/cn-agent-skills --force         # overwrite / update if already installed
```
